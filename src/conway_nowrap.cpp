...
void set_neighbours(unsigned char **grid, int n)
{
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            ...
            if (i != 0) { 
                // LEFT
                if (j != 0 && grid[i-1][j-1] & 0x01) neighbours += 1;

                // CENTER
                if (grid[i-1][j] & 0x01) neighbours += 1;

                // RIGHT
                if (j != n-1 && grid[i-1][j+1] & 0x01) neighbours += 1;
            }

            // LEFT
            if (j != 0 && grid[i][j-1] & 0x01) neighbours += 1;

            // RIGHT
            if (j != n-1 && grid[i][j+1] & 0x01) neighbours += 1;
...
void toggle_cell(unsigned char **grid, int i, int j, int n)
{
    ...
    if (i != 0) {
        // LEFT
        if (j != 0) grid[i-1][j-1] += delta;

        // CENTER
        grid[i-1][j] += delta;

        // RIGHT
        if (j != n-1) grid[i-1][j+1] += delta;
    }

    // LEFT
    if (j != 0) grid[i][j-1] += delta;

    // RIGHT
    if (j != n-1) grid[i][j+1] += delta;
